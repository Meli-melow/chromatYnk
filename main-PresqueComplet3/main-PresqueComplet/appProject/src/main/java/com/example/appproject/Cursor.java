...
public class Cursor {
    ...
    public void forward(int distance){
        //v : value to move forward
        //Translations : x->x+v*cos(direction) ; y->y+v*sin(direction)
    ...
    public void forward(Percentage value, int dimension){
        //dimension is the largest side of the canvas, either width or height
    ...
    public void lookAt(int lookAt_x, int lookAt_y){
        //Check if the entered coordinates equals the cursor's
        //Opposite side
        double u = Math.abs(lookAt_x - getPositionX());
        //Adjacent side
        double v = Math.abs(lookAt_y - getPositionY());

        //We assure the division u/v.
        ...
        if (lookAt_y==getPositionY()) {
            ...
            /*
                ...
                the targeted cursor.(?)
                Angle between horizontal line passing by the choosen cursor and the line going through the cursor
                and the position to look at.
            */
            float angle = (float) Math.toDegrees(Math.atan(u / v));

            /*
                Depending on where the position to look at is, we have to adapt the new angle.
            */

            //the half-plan left to the selected cursor
            if (this.getPositionX()<=lookAt_x){
                //Top left corner
                if (this.getPositionY()>lookAt_y){
                    this.setDirection(angle + 270);
                }
                //Bottom left corner
            ...
            //the half plan right to the selected cursor
            else if (this.getPositionX()>lookAt_x) {
                //Top right corner
                if (this.getPositionY()>lookAt_y){
                    this.setDirection((90-angle) + 180);
                }
                //Bottom right corner
                else if (this.getPositionY()<lookAt_y) {
                    this.setDirection(angle+90);
                }
            }
        }
    }
