...
public class Interpreter {
    ...
    static boolean inFOR = false;

    /**
     * Cursor use in interpret
     */
    static Cursor usedCursor;
    ...
    public static void interpret(String input, Interface interfaceInstance, MapCursor cursors, Cursor cursor, MapVariable variables, int numLine) {
        ...
        if (interfaceInstance.itCounter == 1 ||inFOR){
            usedCursor=cursor;
            inFOR =false;
        }


        // Interrupts the method in case there are no instructions or interruption is called.
        ...
        for (String instruction : instructions) {
            ...
            // When the maximal number of instructions is reached, the execution is stopped.
            ...
            KeyFrame keyFrame = new KeyFrame(Duration.millis(interfaceInstance.executeTime * index), event -> {
                try {
                    ...
                    executeInstruction(currentInstruction, interfaceInstance, cursors, usedCursor, variables, numLine);
    ...
    /**
     * The method launches the execution of the instructions with a temporary cursor.
     *
     * @param input
     * @param interfaceInstance
     * @param cursors
     ...
     * @param tmpCursor
     */
    public static void interpretWithTmpCursor(String input, Interface interfaceInstance, MapCursor cursors, Cursor cursor, MapVariable variables, int numLine, Cursor tmpCursor) {
        List<String> instructions = splitCommand(input);
        ...
        for (String instruction : instructions) {
            ...
            KeyFrame keyFrame = new KeyFrame(Duration.millis(interfaceInstance.executeTime * index), event -> {
                try {
                    ...
                    executeInstruction(currentInstruction, interfaceInstance, cursors, tmpCursor, variables, numLine);
                } catch (Exception e) {
                    interfaceInstance.addHistory("Line " + numLine + " Error executing command: " + currentInstruction + " " + e.getMessage(), Color.RED);
                    e.printStackTrace();
                    stop = true;
        ...
        //We delete the cursor at the end of the instructions
        KeyFrame finalKeyFrame = new KeyFrame(Duration.millis(interfaceInstance.executeTime * index), event -> {
            interfaceInstance.removeCursor(tmpCursor);
        });
                ...
                switch (tokens[0]) {
                    ...
                    case "PRESS":
                        ...
                        if (Num.isName(tokens[1])) {
                            ...
                            Num var = (Num) variables.getVariableById(tokens[1]);
                            opacity = var.getNumValue();
                        } else if (tokens[1].endsWith("%")){
                            Percentage opacity_per = new Percentage(tokens[1]);
                            opacity = opacity_per.getValue();
                        }
                        else {opacity = Double.parseDouble(tokens[1]);}
                    ...
                    case "CURSOR":
                        ...
                        break;
                    case "SELECT":
                        usedCursor = cursors.getCursorById(Integer.parseInt(tokens[1]));
                        interfaceInstance.selectedCursorId = Integer.parseInt((tokens[1]));
                        interfaceInstance.addHistory("Cursor " + usedCursor.getId() + " : " + tokens[0], Color.BLACK);
                        break;
                    case "REMOVE":
                        interfaceInstance.removeCursor(cursors.getCursorById(Integer.parseInt(tokens[1])));
                        interfaceInstance.addHistory("Cursor " + tokens[1] + " : " + tokens[0], Color.BLACK);
                        break;
                    case "FOR":
    ...
    private static void executeFwd(String[] tokens, Interface interfaceInstance, Cursor cursor, MapVariable variables) throws Exception, UndefinedVariableException {
        try {
            int distance;
            //Checks if a variable is being used
            if (Num.isName(tokens[1])) {
                if (!(variables.getVariableById(tokens[1]) instanceof Num)) {
                    throw new UndefinedVariableException("The name is does not refer to a Num variable");
                }
    ...
    private static boolean evaluateBooleanExpression(String condition) throws IllegalArgumentException{
        ...
        for (String logicalPart : logicalParts) {
            ...
            */
            ...
            // Dividing the expression using the comparison expressions.
            ...
            //If it is a comparison
                ...
                if (comparisonParts[0].equals("true") || comparisonParts[0].equals("false") &&
                    ...
                    switch (operator) {
                        case "==" :
                            result = (Boolean.parseBoolean(comparisonParts[0]) == Boolean.parseBoolean(comparisonParts[1]));
                            break;
                        case "!=":
                            result = (Boolean.parseBoolean(comparisonParts[0]) != Boolean.parseBoolean(comparisonParts[1]));
                            break;
    ...
    private static List<String> splitCommand(String instruction) {
        List<String> instructionSplit = new ArrayList<>();
        ...
        for (int i = 0; i < instruction.length(); i++) {
            ...
            if ((character == ';') && (count == 0)) {
                instructionSplit.add(tokens.toString().trim());
                tokens.setLength(0);
            } else {
    ...
    private static void executeMimic(String[] tokens, String instruction, Interface interfaceInstance, MapCursor cursors, Cursor cursor, MapVariable variables,int numLine) throws MimicException {
        if (tokens.length < 2) {
            throw new MimicException("Error : Invalid Mimic syntax");
        }
        /*
        ...
        for (String command : commands) {
            try {
                interpretWithTmpCursor(command, interfaceInstance, cursors, cursors.getCursorById(modelCursorId),variables,numLine,tmpCursor);
            } catch (Exception e) {
                interfaceInstance.removeCursor(tmpCursor);
     ...
     *
     * Version  1 : the temporary cursor is symmetrical to the chosen cursor by using a central symmetry
     * with the coordinates of a chosen point.
     *
     * Version 2 the temporary cursor is symmetrical to the chosen cursor by using a symmetrical axis
     * with the coordinates of two chosen points.
     *
     * @param tokens
     * @param instruction
     * @param interfaceInstance
    ...
    private static void executeMirror(String[] tokens, String instruction, Interface interfaceInstance, MapCursor cursors, Cursor cursor,MapVariable variables,int numLine) throws Exception {
        ...
        if (tokens[3].contains("{")) {
            ...
            double tmpX=2 * symetryPointX - cursor.getPositionX();
            double tmpY = 2 * symetryPointY - cursor.getPositionY();
            interfaceInstance.checkPosition(tmpX,tmpY);
            ...
            tmpCursor.setPositionX((int) Math.round(tmpX));
            tmpCursor.setPositionY((int) Math.round(tmpY));
            tmpCursor.turn(180);
            interfaceInstance.drawCursor(tmpCursor);
            if (!instruction.contains("{") && !instruction.contains("}")){
                interfaceInstance.removeCursor(tmpCursor);
            ...
            try {
                ...
                * The block is executed for the selected cursor and then for temporary one.
                 */
                interpretWithTmpCursor(Block, interfaceInstance, cursors, cursor, variables,numLine,tmpCursor);
    ...
    private static void executeStr(String[] tokens, Interface interfaceInstance, MapVariable vars) {
        //Checks the variable has a valid name
        Variable.isValidName(tokens[1]);
        try {
            String newId = tokens[1];
            if(vars.containsKey(newId)){
